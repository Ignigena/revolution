...
- (void)setDefaults
{
    ...
    [self setGradientStartColor:[NSColor colorWithCalibratedWhite:0.94 alpha:1.0]];
    [self setGradientEndColor:[NSColor colorWithCalibratedWhite:0.79 alpha:1.0]];
...
- (NSBezierPath *)titlePathWithinRect:(NSRect)rect cornerRadius:(float)radius titleRect:(NSRect)titleRect
{
    ...
    if ([[self title] isEqualToString:@""]) {
		// There is no title, so we should draw no titlebar
    } else if (bgRect.size.width - titleRect.size.width >= titleExpansionThreshold && ![self drawsFullTitleBar]) {
...
// When the mouse is clicked, toggle box selection, gradient, and full titlebar
// TO DO: Toggle only when selecting a DIFFERENT box instead of the same one

- (void)mouseDown:(NSEvent *)theEvent {
	if (![self selected]) { selected = YES; } else { selected = NO; }
	if (![self drawsFullTitleBar]) { drawsFullTitleBar = YES; } else { drawsFullTitleBar = NO; }
	if (![self drawsGradientBackground]) { drawsGradientBackground = YES; } else { drawsGradientBackground = NO; }

    [self setNeedsDisplay:YES];
}

// TO DO: When mouse goes over box, turn gradient ON
